...
class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def create_profile
    devise_sessions = session['devise.regist_data']
    @profile = Profile.new(profile_params)
    unless @profile.valid?
      flash.now[:alert] = @profile.errors.full_messages
      render :new_profile and return
    end
    devise_sessions[:profile] = @profile.attributes
    session['devise.regist_data'] = devise_sessions
  ...
  def create_address
    ...
    user.save
    sign_in(:user, user)
    redirect_to root_path, notice: '登録成功しました。'
  end
